...
import { FiPlus, FiEdit2, FiTrash2, FiArrowUp, FiArrowDown, FiAward, FiCoffee, FiTrendingUp, FiCheckCircle, FiHeart, FiGift, FiTruck } from 'react-icons/fi';
...
const FeaturesPanel = () => {
  ...
  const { data: response, isLoading, isFetching } = useGetFeaturesQuery(filters);
  ...
  const openAddModal = () => {
    ...
    reset({ icon: 'FiAward', title: '', description: '', is_active: true });
    setIsModalOpen(true);
  };
  ...
  const onSubmit = async (values) => {
    try {
      if (editingFeature) {
        await updateFeature({ id: editingFeature.id, ...values }).unwrap();
        toast.success('Feature card updated successfully!');
      } else {
        await createFeature({ ...values, sort_order: (response?.data?.length || 0) }).unwrap();
        toast.success('Feature card created successfully!');
      }
      handleCloseModal();
    } catch (e) {
      toast.error(e.data?.message || 'Failed to save feature');
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this feature card?')) {
      try {
        ...
        toast.error(e.data?.message || 'Failed to delete feature');
      }
    }
  };
  ...
  const handleMove = async (index, direction) => {
    ...
    try {
      await reorderFeatures({ ids: list.map((item) => item.id) }).unwrap();
  ...
  const columns = [
    {
      ...
      header: 'Icon',
      render: (row) => {
        const IconComponent = iconMap[row.icon] || FiAward;
        return (
          <div className="w-10 h-10 rounded-lg bg-orange-100 flex items-center justify-center text-accent">
    ...
    {
      key: 'title',
      header: 'Feature Card Title',
      render: (row) => (
        <div>
          <div className="font-semibold text-slate-800">{row.title}</div>
          <div className="text-xs text-slate-400 max-w-md line-clamp-1">{row.description}</div>
        </div>
      ),
    },
    {
            ...
            >
              <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <h3 className="text-lg font-bold text-ink">
                  {editingFeature ? 'Edit Feature Card' : 'Add Feature Card'}
                </h3>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="p-6 flex flex-col gap-5">
                <div className="flex flex-col gap-2">
                  <label className="text-sm font-semibold text-slate-600">Feature Card Title</label>
                  <input
                    type="text"
                    required
                    className="px-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent/20"
                    placeholder="e.g. 100% Organic Recipes"
                    {...register('title', { required: true })}
                  />
                </div>

                <div className="flex flex-col gap-2">
                  <label className="text-sm font-semibold text-slate-600">Icon Type</label>
                  <select
                    ...
                    {...register('icon')}
                  >
                    <option value="FiAward">Award / Quality Medal</option>
                    <option value="FiCoffee">Coffee / Dining Drinks</option>
                    <option value="FiTrendingUp">Trending / Growing Popularity</option>
                    <option value="FiCheckCircle">CheckCircle / Clean Hygiene</option>
                    <option value="FiHeart">Heart / Sincere Hospitality</option>
                    <option value="FiGift">Gift / Exquisite Offers</option>
                    <option value="FiTruck">Truck / Super Fast Delivery</option>
                  </select>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <textarea
                    ...
                    placeholder="Describe this selling point in detail..."
                ...
                <div className="flex items-center gap-3 mt-2">
                  <input
                    type="checkbox"
                    id="checkbox-feature-is-active"
                    className="w-4.5 h-4.5 border border-slate-200 rounded text-accent focus:ring-accent"
                    {...register('is_active')}
                  />
                  <label htmlFor="checkbox-feature-is-active" className="text-sm font-semibold text-slate-700 cursor-pointer">
                    Is Active & Visible
                  </label>
                </div>
