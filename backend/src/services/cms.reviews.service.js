...
const SELECT_FIELDS = 'id, quote, author_name, author_handle, visit_count, rating, sort_order, is_active, created_at, updated_at';

/**
 * Service for cms_reviews (orderable list).
 */
class CmsReviewsService {
    async getAll(query = {}) {
        ...
        if (search)            q = q.ilike('author_name', `%${search}%`);
    ...
    async reorder(ids) {
        await Promise.all(ids.map((id, index) =>
            supabase.from('cms_reviews').update({ sort_order: index }).eq('id', id)
        ));
    }
}

module.exports = new CmsReviewsService();
