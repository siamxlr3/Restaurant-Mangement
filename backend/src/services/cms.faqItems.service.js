...
const SELECT_FIELDS = 'id, question, answer, sort_order, is_active, created_at, updated_at';

/**
 * Service for cms_faq_items (orderable list).
 */
class CmsFaqItemsService {
    async getAll(query = {}) {
        ...
        if (error) throw new Error(`Failed to list FAQ items: ${error.message}`);
    ...
    async getById(id) {
        const { data, error } = await supabase
            ...
            .is('deleted_at', null)
            .single();
        if (error) throw new Error(`FAQ item not found: ${error.message}`);
        return data;
    }
    ...
    async reorder(ids) {
        await Promise.all(ids.map((id, index) =>
            supabase.from('cms_faq_items').update({ sort_order: index }).eq('id', id)
        ));
    }
}

module.exports = new CmsFaqItemsService();
